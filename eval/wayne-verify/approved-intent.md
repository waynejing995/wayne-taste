...
| VV11 | Accept only a legitimate `E2E: none`; reject a skip hiding a user-visible path without inventing a replacement row; a legitimate skip still reaches the final gate | `fe578b0:wayne-verify/SKILL.md` Process Flow; `_shared/e2e-contract.md` Trigger Rule; current A/K | blind requirement/trace review; legitimate skip passes without execution while suspect skip stays unchanged and blocked | legit-skip, suspect-skip | FROZEN |
