...
|---|---|---|---|---|---|---|---|
| I1 | Handoff returns a packet and never invokes the next stage | `wayne-checkpoint/SKILL.md@fe578b0`, Mode A and Step 5 | intended | Skill | blind AI review of the packet and trace; Git mutation evidence | all internal | VERIFIED |
| I2 | The handoff packet carries snapshot, one next agent, self-contained prompt, optional goal, and manual trigger | `wayne-checkpoint/SKILL.md@fe578b0`, Step 3 | intended | template | blind AI review of the complete packet and source artifacts | all internal | VERIFIED |
| I3 | Existing plan handoff routes to `wayne-work` | `wayne-checkpoint/SKILL.md@fe578b0`, Step 2 route table | intended | Skill | blind route review; observed fields and Skill existence are evidence | plan-regression | VERIFIED |
...
| I8 | Every `next_agent` is one real Skill slug, never a verdict, chain, or external owner | `wayne-checkpoint/templates/handoff-packet.md@fe578b0`; repository skill directories | control defect | caller + checkpoint | blind route review supported by real Skill-existence evidence | all internal | VERIFIED |
...
| I11 | Checkpoint and handoff templates carry path/owner/hash references plus derived progress only; they never become a second decision, U Status, or E Status owner | `_shared/pipeline-id-contract.md` Artifact state and field owners; current Save ownership rule | intended ownership repair | templates | blind AI ownership review; checker observations may locate copied state but cannot decide meaning | static templates | FROZEN |
...
packet packaging and routing preservation. Markdown layout is guidance, not a second
contract owner.
