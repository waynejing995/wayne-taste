...
| TD02 | Matrix has U-SEED owned provisionally here and locked later by plan, plus E rows owned here with distinct status authorities; when no stable U seed exists the exact owner label remains with an explicit none reason | `cbe9307:wayne-test-design/SKILL.md` authorship and checklist step 6 | exact U-SEED label plus `☐` rows or `U-SEED: none — reason`; locked E header/`⬜`; no `☑/✅/❌` | all | FROZEN |
...
| TD09 | Locked E lifecycle stays `⬜ → ✅/❌`; isolation never invents statuses or columns | owner draft rule 6; `_shared/e2e-contract.md` | exact seven-column header; initial `⬜` only | all | FROZEN |
...
| TD16 | Absorb either an existing E table or `E2E: none — reason` verbatim exactly once, then extend missing observable paths | `636c81e:wayne-test-design/SKILL.md` Phase 3 | calibrated static absorption guard; absorb-existing checks table literal | absorb-existing/static | FROZEN |
