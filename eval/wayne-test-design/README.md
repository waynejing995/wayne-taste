...
Each Claude/Codex trial receives one isolated skill snapshot and approved spec,
writes only `docs/test-matrix/matrix.md`, and is scored by `check_trial.py`.
...
and Codex separately with `run_agent.sh`, then apply `check_trial.py`. Never reuse a
workspace, state directory, or output from an invalidated harness revision.
