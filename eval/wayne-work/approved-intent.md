...
|---|---|---|---|---|---|---|---|
| W1 | Validate complete, non-conflicting plan/decision/matrix/spec before editing | `wayne-work/SKILL.md@cbe9307`, Inputs; current A | intended | main | AI review of blocker reason/artifacts/owner plus zero mutation | protected, missing-u | VERIFIED |
...
| W6 | Each worker receives one full unit contract, allowed paths, exact verify, no-commit, and no-matrix/shared-owner boundary | `wayne-work/SKILL.md@cbe9307:188-210,234-243` | intended | main dispatch | blind worker-prompt review plus external call/result trace | parallel-disjoint | VERIFIED |
...
| W9 | Unit exact verification must pass before its U rows change `☐→☑`; E stays `⬜` | `wayne-work/SKILL.md@636c81e,cbe9307`; current G/H | intended | main owns status | blind matrix/ordering review plus real test-event evidence | normal, parallel-disjoint | VERIFIED |
...
| W11 | Final completion proves all units/decisions, scope diff, exact full commands, and no incomplete/staged/committed/branched work | current J and Red lines | intended | main | hidden tests, starting Git state, agent write history, final diff, full verify | normal, parallel-disjoint | VERIFIED |
| W12 | Success emits return-only checkpoint to `wayne-code-review` and never invokes it | `wayne-work/SKILL.md@fe578b0`; current L | intended | checkpoint packet | blind packet/trace review supported by file and Skill evidence | normal, parallel-disjoint | VERIFIED |
