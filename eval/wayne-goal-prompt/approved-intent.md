...
| GP10 | App-server startup/provider failure is reported before a successful job ID | fail-loud policy; current “Assuming dispatch landed” anti-pattern | fake initialize failure makes `dispatch` non-zero and preserves logs | dispatch-failure | PASS |
