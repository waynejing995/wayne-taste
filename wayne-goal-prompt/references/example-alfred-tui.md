...
The reference goal prompt this skill's anatomy is anchored on. Note how every constraint is a "Do not" red-line inlined at the task it governs, verification names exact commands + a real pty/xterm path, and completion criteria are each testable. Use it as the bar a composed prompt must clear.
