...
Use this skeleton when it makes the goal easier to execute. Preserve the required information, but adapt headings or grouping when clearer. Delete the parenthetical hints and emit one copy-paste block. Current correction is by-need.
...
- (secret hygiene: never print/copy secret values; materialize via an env-var name like ALFRED_X_HEADERS, pass only the env-var reference.)
...
- (a REAL e2e path driven through the actual entrypoint — name it; forbid the fake substitute, e.g. "do not replace the TUI drive with direct CLI calls".)
