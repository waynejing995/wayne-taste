...
**Proof layer** — where the evidence is taken. When the User path names a person doing
something on a surface, that surface is the layer, and evidence from below it never
closes the row.
...
`POST /design-runs` returning 201 does not prove the button calls it. A row phrased as a
user capability was closed exactly that way while the button was a `<Link>` to a tab.

**Kind** — what the row asserts.

- `goal-walk` — one whole user goal, walked end to end against a live stack at that
  user's own layer, and it is the gate. Name the goal, not the screen: "an engineer
  turns a conversation into a spec", not "the run page renders". It asserts
  **arriving**; if the walk cannot reach the end, the feature is not done however many
  components pass. Exactly one per distinct user goal.
- `edge` — one UI control and the state change it produces: a URL change, a dialog
  appearing, a row appearing in a list. Model the UI as a graph: nodes are reachable
  states, edges are the actions between them, and the rows are the **edges**. A matrix
  made only of nodes cannot express "this button does nothing", so a dead control is
  never tested. A control that produces no state change is a **failed** edge, not an
  untested one.
- `path` — a supporting journey that is neither a whole goal nor a UI control: a
  prerequisite that can fail on its own, an attestation, an aggregate, a resilience or
  cleanup route. These carry the proof-axis work and are unchanged by the other two.

`goal-walk` and `edge` do not substitute for each other. Edge coverage enumerates what
EXISTS, so it cannot see a control that was never written — a product whose central
mechanism had no client anywhere still scored 100% control coverage. A goal-walk cannot
say which control is dead. A UI needs both.
