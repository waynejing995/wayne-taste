...
That reason is also the limit. `flowchart` and `sequenceDiagram` are the portable profile: they are what every host has shipped for years, and a newer diagram type renders in the live editor and as an error box in the reader's. For the same reason a spec carries no in-block renderer configuration — no `%%{init: ...}%%` directive and no `--- config: ... ---` frontmatter. `layout: elk` is the one worth naming, because it is the tempting one: ELK produces better routing on a dense flowchart, but it ships as `@mermaid-js/layout-elk` and the host has to register it, which GitHub and core Obsidian do not. The diagram then silently falls back to dagre for the actual readers while looking correct wherever it was authored. A flowchart that only reads well under ELK is a flowchart with too many nodes; split it into the levels this section already requires.

Quote every node and edge label — `A["Open decision log"]`, not `A[Open decision log]`. An unquoted label parses right up until one of them contains a bracket, paren, comma, or colon, and then the whole diagram fails; quoting by habit keeps you off that path instead of discovering it one label at a time. What is actually enforced is narrower and runs at node M: [the mermaid checker](../scripts/check_mermaid.mjs) parses every block with the real grammar and rejects a non-portable diagram type or an in-block renderer config. It runs locally, so none of this depends on pasting the spec into an online renderer.
